...
class DefaultFacebookCallback(BaseFacebookCallback):
    ...
    def persist(self, user, token, user_data):
        ...
        if not created:
            ...
            assoc.save()
            logger.debug("DefaultFacebookCallback.persist: UserAssociation object updated")
        else:
            logger.debug("DefaultFacebookCallback.persist: UserAssociation object created")
    ...
    def login_user(self, request, user):
        ...
        logger.debug("DefaultFacebookCallback.login_user: logging in user %s with ModelBackend" % str(user).strip())
        login(request, user)
